class Wheel {
  constructor(parentID, data, count = 5, startIndex = 0) {
    ...
    // this.backgroundColor = select(this.parentID).style('background-color');
    ...
    for (let i = 0; i < this.count; i++) {
      this.elements[i] = createP(
        this.data[
          (this.startIndex + i - floor(this.count / 2)) % this.data.length
        ]
      );
  ...
  move() {
    ...
    let howfar = this.target - this.startIndex;
    if (howfar < 0) {
      howfar = this.target + this.data.length - this.startIndex;
    }
    this.speed = 0.2 * howfar;
    ...
    if (this.currTop < -this.distanceBetweenElements / 2) {
      ...
      this.elements.push(
        createP(
          this.data[
            (this.data.length + this.startIndex + floor(this.count / 2)) %
              this.data.length
          ]
        )
      );
    ...
    if (this.target == this.startIndex) {
      this.spin = false;
      for (let i = 0; i < this.elements.length; i++) {
